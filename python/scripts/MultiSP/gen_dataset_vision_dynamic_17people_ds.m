listing = dir('..\..\lips_extracting\lips_24_16\500ms_prets\CSVwords_visual_dyn');
sx=24; %32;
sy=16; %24
dataset1 = zeros (sx*sy,100);
dataset2 = zeros (sx*sy,100);
dataset3 = zeros (sx*sy,100);
dataset4 = zeros (sx*sy,100);
dataset5 = zeros (sx*sy,100);
dataset6 = zeros (sx*sy,100);
dataset7 = zeros (sx*sy,100);
dataset8 = zeros (sx*sy,100);
dataset9 = zeros (sx*sy,100);
datasetz = zeros (sx*sy,100);
dataseto = zeros (sx*sy,100);
...
for i=1:a
    if listing(i).isdir == 0
        if strcmp(listing(i).name(end-8:end),'_hist.csv')
             listing(i).name
             fname = ['..\..\lips_extracting\lips_24_16\500ms_prets\CSVwords_visual_dyn\' listing(i).name];
             if (strfind(listing(i).name,'one'))
                 dataset1(:,i1)=csvf2line_dyn(fname, listing(i).name, 'one'); i1=i1+1;
             end
             if (strfind(listing(i).name,'two'))
                 dataset2(:,i2)=csvf2line_dyn(fname, listing(i).name,'two'); i2=i2+1;
             end
             if (strfind(listing(i).name,'three'))
                 dataset3(:,i3)=csvf2line_dyn(fname, listing(i).name,'three'); i3=i3+1;
             end
             if (strfind(listing(i).name,'four'))
                 dataset4(:,i4)=csvf2line_dyn(fname, listing(i).name,'four'); i4=i4+1;
             end
             if (strfind(listing(i).name,'five'))
                 dataset5(:,i5)=csvf2line_dyn(fname, listing(i).name,'five'); i5=i5+1;
             end
             if (strfind(listing(i).name,'Six'))
                 dataset6(:,i6)=csvf2line_dyn(fname, listing(i).name,'Six'); i6=i6+1;
             end
             if (strfind(listing(i).name,'Seven'))
                 dataset7(:,i7)=csvf2line_dyn(fname, listing(i).name,'Seven'); i7=i7+1;
             end
             if (strfind(listing(i).name,'Eight'))
                 dataset8(:,i8)=csvf2line_dyn(fname, listing(i).name,'Eight'); i8=i8+1;
             end
             if (strfind(listing(i).name,'Nine'))
                 dataset9(:,i9)=csvf2line_dyn(fname, listing(i).name,'Nine'); i9=i9+1;
             end
             if (strfind(listing(i).name,'Zero'))
                 datasetz(:,iz)=csvf2line_dyn(fname, listing(i).name,'Zero'); iz=iz+1;
             end
             if (strfind(listing(i).name,'Oh'))
                 dataseto(:,io)=csvf2line_dyn(fname, listing(i).name,'Oh'); io=io+1;
             end
             i
        end
    end
...
csvwrite('.\\ds\\dataset_visual_lips_24x16_in_train.csv',datasetin_tr',0,0);
csvwrite('.\\ds\\dataset_visual_lips_24x16_out_train.csv',datasetout_tr',0,0);
csvwrite('.\\ds\\dataset_visual_lips_24x16_in_val.csv',datasetin_va',0,0);
csvwrite('.\\ds\\dataset_visual_lips_24x16_out_val.csv',datasetout_va',0,0);