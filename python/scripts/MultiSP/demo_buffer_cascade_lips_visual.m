...
clc 
%%%%%%%%%%%%%%%%%%%% SNN Training %%%%%%%%%%%%%%%%%%%%%%%%%%
file=transpose(csvread('ds\\dataset_visual_lips_64x48_in_600_train.csv', 0,0)); %% input to SNN 
ti= num2cell(file,3);
td=transpose(csvread('ds\\dataset_visual_lips_64x48_out_600_train.csv', 0,768));  %% expected output (supervised learning)
net=newsnn([768 50 10 1],1:10,[0 5 3 0],'wim1');
...
wg=getx_snn(net);
wg=wg*1000;
Test_file=csvread('ds\\xAqib\\dataset_visual_lips_64x48_in_test.csv', 0,0);  %% input to SNN 
Test_ti= transpose(num2cell(Test_file,3));
test_td=csvread('ds\\xAqib\\dataset_visual_lips_64x48_with_output_test.csv', 0,768);   %% desire output 
a=size(Test_file);
Train_size= a(:,1);
for t=1:Train_size
